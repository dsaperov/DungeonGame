# -*- coding: utf-8 -*-
"""
DungeonGame - консольная игра, написаная в рамках домашнего задания к 15 модулю курса Python-разработчик от Skillbox.

С помощью JSON файла rpg.json задана "карта" подземелья. Подземелье было выкопано монстрами и они всё ещё скрываются
где-то в его глубинах, планируя набеги на близлежащие поселения. Само подземелье состоит из двух главных разветвлений и
нескольких развилок, и лишь один из путей приведёт вас к главному Боссу и позволит предотвратить набеги и спасти
мирных жителей.

Пользователь может исследовать подземелье, перемещаясь из одной локации в другую, однако при перемещении в любую
локацию, кроме стартовой, главный персонаж теряет время, указанное в конце названия каждой локации. Так, перейдя в
локацию Location_1_tm500 - со счета будет списано 500 секунд.

Также можно сражаться с монстрами. После битвы со счета персонажа также списывается N-количества времени и начисляется
N-количества опыта. Опыт и время указаны в названиях монстров (после exp указано значение опыта и после tm указано
время). Так, если бвло српжение с монстром Mob_exp10_tm20 (или Boss_exp10_tm20), то со счета будет списано 20 секунд и
добавлено 10 очков опыта.

Для победы в игре необходимо набрать 280 очков опыта при положительном остатке времени.

По мере прохождения игры ведется лог в формате .csv, в котором содержится следующая информация:
-- текущее положение
-- текущее количество опыта
-- время, прошедшее с момента начала игры (по условиям задания подсчет необходимо вести с помощью модуля datetime)
"""
...
class Warrior:
    ...
    DEFAULT_TIME_LEFT = 1234567890.1

    def __init__(self):
        self.time_left = self.DEFAULT_TIME_LEFT
        self.experience = 0
...
class GameCore:
    ...
    def handle_start_new_game_choice(self, choice):
        if choice == 'y':
            ...
            self.hero.time_left = self.hero.DEFAULT_TIME_LEFT
